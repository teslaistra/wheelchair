...
if __name__ == "__main__":
        ...
        while not rospy.is_shutdown():
            ...
            if drive_control.get(str(dec2hex(msg.arbitration_id, 8))) != None:
                ...
                data = binascii.hexlify(msg.data)
                text = "Recieved message about/from: " + event_messages.get(str(dec2hex(msg.arbitration_id, 8))) + "with data" + str(binascii.hexlify(msg.data))
                msg= String()
                msg.data = text
                EventPublisher.publish(msg)
            elif periodic_messages.get(str(dec2hex(msg.arbitration_id, 8))) != None:
                data = binascii.hexlify(msg.data)
                text = "Recieved periodic message about/from: " + event_messages.get(str(dec2hex(msg.arbitration_id, 8))) + "with data" + str(binascii.hexlify(msg.data))
                msg = String()
                msg.data = text
